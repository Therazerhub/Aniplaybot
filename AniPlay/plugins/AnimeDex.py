...
class AnimeDex:
    ...
    def search(query):
        url = "https://api.bhavrevaibhav.workers.dev/search/" + str(
    ...
    def anime(id):
        data = requests.get("https://api.bhavrevaibhav.workers.dev/anime/" + id).json()[
            "results"
        ]
    ...
    def episode(id):
        data = requests.get("https://api.bhavrevaibhav.workers.dev/episode/" + id).json()[
            "results"
        ]
        text = data["name"]
        surl = [
    ...
    def download(id):
        data = requests.get("https://api.bhavrevaibhav.workers.dev/download/" + id).json()[
            "results"
        ]
        return data
